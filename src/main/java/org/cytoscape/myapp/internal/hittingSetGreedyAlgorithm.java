...
public class hittingSetGreedyAlgorithm {
    ...
    */
    ...
    /**
     * Shows final output of greedy Algorithm, each of Vector<Integer> shows one of hitting set solution that contains hashing map of all nodes on corresponding solution.
    */
    private final Vector<Vector<Integer>> CIset;

    /**
     * Shows all of paths between each of source nodes to each of target nodes, each of Vector<Integer> shows one path that contains hashing map of all nodes on this path.
    */
    private final Vector<Vector<Integer>> elementarypaths;
    ...
    */
     ...
     */
    public hittingSetGreedyAlgorithm( int maxCIsize, double maxNumberofCandidate, Vector<Vector<Integer>> elementarypaths,   Vector<Float> ocsanaScore, int[] setscoreset, Vector<Integer> elementaryNodes) {
    ...
    private void kEqualOne() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (count == elementryPathArray.length - 1) {
                temp = new Vector<>();
                temp.addElement(elementaryNodes.get(i));
                CIset.addElement(temp);
        ...
        for (int i = 1; i < c2nprime + 1; i++) {
           ...
           //Elementarypathhit shows how many of paths covered by these two nodes.
            ...
            if (elementarypathhit == elementarypaths.size()) {
                CIset1.addElement(NprimeOrder.get(tempvalue1));
                CIset1.addElement(NprimeOrder.get(tempvalue2));
                CIset.addElement(CIset1);

            } 
            //Otherwise we add them to Useed for create CIset size three or more. else { tempvector = new Vector<>(); tempvector.add(NprimeOrder.get(tempvalue1)); tempvector.add(NprimeOrder.get(tempvalue2)); Useed.add(tempvector);
            else {
  ...
  if ((k == maxCIsize - 1) && exceptionone()){
     ...
     * @param 
     */
    public Vector<Vector<Integer>>  getCIset(){
